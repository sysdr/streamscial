...
export class DataGenerator {
  ...
  async start(): Promise<void> {
    ...
    console.log('✓ Data generator started - producing ~2 events/sec (slow but visible)');
  ...
  private generateTweets(): void {
    ...
    }, 2000); // 1 event/sec (slow but visible)
  }
  ...
  private generateEngagements(): void {
    setInterval(async () => {
      ...
      await this.producer.sendBatch({ topicMessages: batch });
    }, 3000); // 1 event/sec (slow but visible)
  }
  ...
  private generateFollows(): void {
    setInterval(async () => {
      ...
      const batch = Array.from({ length: 1 }, () => {
      ...
      await this.producer.sendBatch({ topicMessages: batch });
    }, 30000); // 0.17 events/sec (very slow)
  }
